// visions.jsx — procedural generators that draw onto a canvas.
// Each generator: (ctx, size, palette, rand) => void
// Pick a palette + generator at random; result is a square image.

const PALETTES = [
  // mystical / oracle vibe — 4-5 colors each
  ['#0a0014', '#3a0066', '#a83cf7', '#ff5fd2', '#ffe8a0'],          // deep amethyst
  ['#001218', '#003844', '#0a8e9e', '#48cae4', '#caf0f8'],          // cyan abyss
  ['#160d05', '#5b1f0a', '#c75a1a', '#ffa653', '#fff1c1'],          // ember
  ['#02110a', '#073c1f', '#1f8d4f', '#9fe870', '#f0ffd6'],          // jade
  ['#0d0518', '#2b1457', '#6d3df5', '#3ddfff', '#fdd0ff'],          // dream
  ['#10040c', '#4a0830', '#c01661', '#ff7aa2', '#ffe1d0'],          // ruby
  ['#070713', '#1a1f4b', '#3d6fff', '#7fc7ff', '#e0f0ff'],          // sapphire
  ['#0a0a0a', '#1a1a1a', '#7a6a48', '#c8a45c', '#f4d987'],          // gold smoke
  ['#020014', '#0d0a3a', '#5a2eb8', '#ff3ab2', '#ffe8ff'],          // nebula
  ['#0f0a05', '#3c1f0a', '#a05a14', '#e6a02e', '#ffe9b3'],          // amber
  ['#000814', '#001d3d', '#003566', '#ffc300', '#ffd60a'],          // royal
  ['#1b0a14', '#4f1a3a', '#a23f74', '#f06ba0', '#ffd9d9'],          // rose dust
];
...
// Add film-grain noise across the whole image at the end of each generator
function grain(ctx, size, r, amount = 0.06) {
...
// ─── 1. NEBULA — billowing radial clouds ───
function nebula(ctx, size, palette, r) {
...
// ─── 2. AURORA — sine-warped bands ───
function aurora(ctx, size, palette, r) {
  ctx.fillStyle = palette[0];
  ctx.fillRect(0, 0, size, size);
...
// ─── 3. COLOR FIELD — Rothko-style soft blocks ───
function colorField(ctx, size, palette, r) {
  ctx.fillStyle = palette[0];
  ctx.fillRect(0, 0, size, size);
  // 2-4 horizontal blocks of color
  ...
  // soft horizontal blur look via overlay grain
  grain(ctx, size, r, 0.08);
}

// ─── 4. IRIDESCENCE — oil-slick conic / diagonal sweep ───
function iridescence(ctx, size, palette, r) {
  const angle = r() * TAU;
  const cx = size / 2, cy = size / 2;
  // simulate conic by drawing radial wedges
...
// ─── 5. TOPO — flow-field contour lines ───
function topo(ctx, size, palette, r) {
  ctx.fillStyle = palette[0];
  ctx.fillRect(0, 0, size, size);
  const lineColor = hexToRgb(palette[palette.length - 2]);
  const accent = hexToRgb(palette[palette.length - 1]);
  const cx = size * (0.3 + r() * 0.4);
  const cy = size * (0.3 + r() * 0.4);
  const cx2 = size * (0.3 + r() * 0.4);
  const cy2 = size * (0.3 + r() * 0.4);
  ...
  for (let i = 0; i < lines; i++) {
    ...
    for (let a = 0; a <= TAU + 0.05; a += 0.02) {
      ...
      const ox = lerp(cx, cx2, t);
      const oy = lerp(cy, cy2, t);
      const x = ox + Math.cos(a) * rad;
      const y = oy + Math.sin(a) * rad;
...
// ─── 6. VORONOI — cellular stained-glass ───
function voronoi(ctx, size, palette, r) {
  const sites = [];
  const n = 18 + Math.floor(r() * 16);
  for (let i = 0; i < n; i++) {
    sites.push({ x: r() * size, y: r() * size, c: hexToRgb(pick(palette.slice(1), r)) });
  }
  const step = 4; // chunky for speed
  ...
  // dark grout via edge detection on second pass
  const img = ctx.getImageData(0, 0, size, size);
  const d = img.data;
  const w = size;
  for (let y = 1; y < size - 1; y++) {
    for (let x = 1; x < size - 1; x++) {
      ...
      const ir = (y * w + x + 1) * 4;
      const ib = ((y + 1) * w + x) * 4;
      if (Math.abs(d[i] - d[ir]) + Math.abs(d[i] - d[ib]) > 50) {
        d[i] = d[i + 1] = d[i + 2] = 8;
      }
    }
  }
...
// ─── 7. STARFIELD — deep space with bloom ───
function starfield(ctx, size, palette, r) {
  // radial dark background
  const cx = size / 2, cy = size / 2;
  const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, size * 0.7);
  const inner = hexToRgb(palette[1]);
  const outer = hexToRgb(palette[0]);
  g.addColorStop(0, rgba(inner, 0.55));
  ...
  for (let i = 0; i < 6; i++) {
    ...
    gg.addColorStop(0, rgba(c, 0.4));
    gg.addColorStop(1, rgba(c, 0));
    ctx.fillStyle = gg;
    ctx.beginPath(); ctx.arc(x, y, rad, 0, TAU); ctx.fill();
  }
  // stars at varying sizes
...
// ─── 8. LIQUID — metaballs / lava lamp ───
function liquid(ctx, size, palette, r) {
  const c0 = hexToRgb(palette[0]);
  const c1 = hexToRgb(palette[1]);
  const g = ctx.createLinearGradient(0, 0, 0, size);
  g.addColorStop(0, rgba(c1, 1));
  g.addColorStop(1, rgba(c0, 1));
  ...
  for (const b of blobs) {
    ...
    ctx.fillStyle = gg;
    ctx.beginPath(); ctx.arc(b.x, b.y, b.rad, 0, TAU); ctx.fill();
  }
  // a few hot specular highlights
  for (const b of blobs) {
    ...
    ctx.fillStyle = gg;
    ctx.beginPath(); ctx.arc(hx, hy, b.rad * 0.35, 0, TAU); ctx.fill();
  }
  ctx.globalCompositeOperation = 'source-over';
  grain(ctx, size, r, 0.04);
}

// ─── 9. RIBBONS — flowing parametric ribbons ───
function ribbons(ctx, size, palette, r) {
  ctx.fillStyle = palette[0];
  ctx.fillRect(0, 0, size, size);
  const ribbons = 6 + Math.floor(r() * 5);
  ctx.globalCompositeOperation = 'lighter';
  for (let i = 0; i < ribbons; i++) {
    ...
    g.addColorStop(0, rgba(c, 0));
    g.addColorStop(0.5, rgba(c, 0.95));
    g.addColorStop(1, rgba(c, 0));
    ctx.fillStyle = g;
    ctx.fill();
  }
...
// ─── 10. SMOKE — turbulent plumes ───
function smoke(ctx, size, palette, r) {
  ctx.fillStyle = palette[0];
  ctx.fillRect(0, 0, size, size);
  ...
  for (let i = 0; i < plumes; i++) {
    ...
    const y = size * (0.3 + r() * 0.5);
    const rad = size * (0.08 + r() * 0.18);
    const c = hexToRgb(pick(palette.slice(1), r));
    const gg = ctx.createRadialGradient(x, y, 0, x, y, rad);
    gg.addColorStop(0, rgba(c, 0.25));
    gg.addColorStop(0.5, rgba(c, 0.12));
    gg.addColorStop(1, rgba(c, 0));
    ctx.fillStyle = gg;
    ctx.beginPath(); ctx.arc(x, y, rad, 0, TAU); ctx.fill();
  }
...
// generate one vision into a canvas; returns the canvas
function generateVision({ size = 1024, generatorId = null, paletteIndex = null, seed = null } = {}) {
  const s = seed ?? (Math.random() * 1e9) | 0;
  const r = mulberry32(s);
  const gen = generatorId
    ? GENERATORS.find(g => g.id === generatorId) || pick(GENERATORS, r)
    : pick(GENERATORS, r);
  const palette = paletteIndex != null
...
window.Visions = { GENERATORS, PALETTES, generateVision };
